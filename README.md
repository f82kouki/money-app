...
2人で「どちらがいくら支払ったか」をスマホからすぐ記録し、合計・差額・**精算額**を確認できる Web アプリ。割り勘（折半）と立て替え（相手が全額負担）の両方に対応し、清算したら集計をリセットできる。

- フロント: React + TypeScript + Vite + Tailwind（スマホ縦画面優先）
- バック: FastAPI + SQLAlchemy（認証は JWT 自作 / **Python 3.12**）
...
- **支払い記録**: 金額・支払った人・項目メモ・日付を記録。**割り勘**（折半）/ **立て替え**（相手が全額返す）を選べる。
- **自動精算**: 「◯◯が△△に □円渡せば均等」を自動計算（端数は1円単位で丸め）。
- **精算してリセット**: 清算したらサマリーの「精算する」で集計を0に戻せる（記録の履歴は残り、「精算済」表示になる）。
- **履歴**: 記録の編集・削除、「もっと見る」で過去分を表示。
- **メッセージ**: グループ内（2人）の簡易チャット。
- **記録時のお祝い画像**: 設定でONにして複数枚登録すると、記録のたびにランダムで1枚表示。記録成功時に効果音（チャリン）も鳴る。
- **アカウント / グループ**: メール(ID)＋パスワードで登録し、招待コードで2人グループに参加。表示名・おさいふ名を編集可能。
- **ログイン保護**: 連続失敗で一時的にロック、ログアウトで発行済みトークンを失効。
...
> backend は **Python 3.12** を前提（`.python-version` / Dockerfile / Vercel と統一）。`make setup-backend` は利用可能な Python を自動検出して venv を作成する。
...
| `make db-init` | DB にテーブル作成・マイグレーション適用 |
| `make backend` / `make frontend` | 片方だけ起動 |
| `make lint` | フロントの型チェック（`tsc --noEmit`） |
| `make build` | フロントの本番ビルド |
| `make check` | requirements同期 + 型チェック + 本番ビルド（デプロイ前） |
| `make check-requirements` | root と backend の requirements.txt のズレを検出 |
...
3. それぞれが支払いを記録（金額・支払った人・項目・日付）。**割り勘**か**立て替え**かを選ぶ。
4. トップのサマリーで各合計・差額・「◯◯が△△に □円渡せば均等」を確認。
5. 実際に清算したら「**精算する**」で集計をリセット（記録の履歴は残る）。
6. 記録は編集・削除可。設定画面で表示名・おさいふ名・お祝い画像を変更でき、メッセージ画面で2人のやり取りもできる。
...
>
> レビュー対応リリース（2026-06）でも事前マイグレーションが必要:
> - `users` に `token_version` / `failed_login_attempts` / `locked_until`（ログイン失効・レート制限）
> - `payments` に `settlement_id`、新規 `settlements` テーブル（精算リセット）
> - `group_members.user_id` に一意インデックス `uq_group_members_user`（重複所属があると作成をスキップし警告するので、その場合は手動クリーンアップ後に再実行）
>
> いずれも `init_db()`（`create_all` + 冪等な `ALTER`/`CREATE INDEX`）で適用される。

## 動作確認シナリオ

アカウントA登録 → グループ作成 → 招待コード取得 → 別ブラウザ/シークレットでアカウントB登録 → コードで参加 → 双方で支払い追加（割り勘・立て替え両方）→ サマリーの合計・差額・精算額を確認 → 「精算する」で集計がリセットされ、履歴に「精算済」が残ることを確認 → 編集・削除 → 設定で表示名・お祝い画像が反映されるか確認。
